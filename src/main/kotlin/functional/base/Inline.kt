...
inline fun <reified T : Any> Iterable<*>.anyOf(): Boolean {
    for (i in this) {
        if (i is T) {
            return true
        }
...
inline fun <reified T : Any> Iterable<*>.firstOfOrNull(): T? {
    for (i in this) {
        if (i is T) {
            return i
        }
...
inline fun <reified T1 : Any, reified T2 : Any> Map<*, *>.filterValuesInstanceOf(): Map<T1, T2> =
    filter { (k, v) -> k is T1 && v is T2 } as Map<T1, T2>
